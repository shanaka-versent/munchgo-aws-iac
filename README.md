...
### CloudFront → Kong API requests timing out

**Symptom:** `/healthz` returns 200 but `/api/*` endpoints timeout or return 504.

**Check NLB target health:**
```bash
# Get NLB target group ARNs
NLB_ARN=$(aws elbv2 describe-load-balancers --query "LoadBalancers[?contains(LoadBalancerName,'kongclou')].LoadBalancerArn" --output text)
aws elbv2 describe-target-groups --load-balancer-arn "$NLB_ARN" --query 'TargetGroups[*].[Port,TargetGroupArn]' --output text | while read PORT ARN; do
  HEALTH=$(aws elbv2 describe-target-health --target-group-arn "$ARN" --query 'TargetHealthDescriptions[0].TargetHealth.State' --output text)
  echo "Port $PORT: $HEALTH"
done
```

**If port 443 is unhealthy:** The `istio-gateway-tls` K8s secret is missing. This is now auto-generated by `03-post-terraform-setup.sh`, but can be run manually:
```bash
kubectl get secret istio-gateway-tls -n istio-ingress  # Check if exists
./scripts/01-generate-certs.sh                          # Recreate if missing
kubectl rollout restart deployment -n istio-ingress     # Restart to pick up secret
```

### SPA deploy fails with `NoSuchBucket`

**Cause:** GitHub CI/CD secrets (`SPA_BUCKET_NAME`, `CLOUDFRONT_DISTRIBUTION_ID`) are stale from a previous stack. After rebuild, bucket names change.

**Fix:** Re-run `03-post-terraform-setup.sh` — it updates all SPA secrets from Terraform outputs automatically. Or manually:
```bash
gh secret set SPA_BUCKET_NAME --body "$(terraform -chdir=terraform output -raw spa_bucket_name)" -R shanaka-versent/munchgo-spa
...
**Check:**
```bash
aws ec2 describe-route-tables --filters "Name=route.destination-cidr-block,Values=192.168.0.0/16" \
  --query 'RouteTables[*].Routes[?DestinationCidrBlock==`192.168.0.0/16`].State' --output text
```

**Fix:** `03-post-terraform-setup.sh` auto-verifies this route. If manual fix is needed:
```bash
terraform -chdir=terraform apply -replace='module.vpc.aws_route.kong_cloud_gateway[0]'
```

### Kong OIDC plugin returning 500

**Check:** Verify the Cognito issuer URL in `deck/kong.yaml` matches the current user pool:
```bash
terraform -chdir=terraform output cognito_issuer_url
grep "issuer:" deck/kong.yaml | head -1
```
If they don't match, re-run `03-post-terraform-setup.sh` which populates the correct issuer URL.

### Pods not receiving traffic after rebuild

**Check the full traffic chain:**
```bash
# 1. CloudFront → Kong proxy (should return 200)
curl -s -o /dev/null -w 'HTTP %{http_code}\n' https://$(terraform -chdir=terraform output -raw application_url)/healthz

# 2. NLB targets healthy
kubectl get gateway -n istio-ingress  # Should show Programmed=True

# 3. HTTPRoutes attached
kubectl get httproute -A              # Should show routes in munchgo namespace
...
|--------|---------|-----------|
| `01-generate-certs.sh` | Generate self-signed TLS certs for Istio Gateway HTTPS listener | `03-post-terraform-setup.sh` (automatic) |
| `02-setup-cloud-gateway.sh` | Attach Transit Gateway to Kong Cloud Gateway Network | Step 4 (manual) |
| `02-generate-jwt.sh` | Generate a Cognito JWT for authenticated API testing | Manual testing |
| `03-post-terraform-setup.sh` | Infrastructure config: TLS certs, placeholders, Kong sync, GitHub secrets | Step 5 (manual) |
| `04-deploy-apps.sh` | Trigger CI, deploy SPA, seed data, smoke tests | Step 6 (manual) |
| `seed-admin-user.sh` | Create admin user in Cognito + auth-service DB | `04-deploy-apps.sh` (automatic) |
| `seed-demo-data.sh` | Seed demo restaurants and menus via API | `04-deploy-apps.sh` (automatic) |
| `destroy.sh` | Full ordered teardown (TGW detach → Terraform destroy → reset) | Teardown (manual) |
